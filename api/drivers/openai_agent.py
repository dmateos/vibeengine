...
class OpenAIAgentDriver(BaseAgentDriver):
    ...
    def execute(self, node: Dict[str, Any], context: Dict[str, Any]) -> DriverResponse:
        ...
        logger.info(f"[OpenAI] Starting execution - Node: {label} ({node_id})")
        logger.debug(f"[OpenAI] Input: {str(input_text)[:200]}...")
        ...
        logger.info(f"[OpenAI] Using model: {model}, temperature: {temperature_val}, base_url: {base_url}")
        logger.debug(f"[OpenAI] API key source: {'node config' if data.get('api_key') else 'env var'}")
        ...
        try:
            ...
            logger.info(f"[OpenAI] Execution completed - Node: {label} ({node_id})")
            logger.debug(f"[OpenAI] Output: {str(content)[:200]}...")
            if call_log:
                logger.info(f"[OpenAI] Tool calls made: {len(call_log)}")
                logger.debug(f"[OpenAI] Tool call log: {json.dumps(call_log)[:500]}...")
            ...
            logger.error(f"[OpenAI] Error in node {label} ({node_id}): {error_msg}")
